...
export default function GradeAssessmentPage() {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    setError(false)
    try {
      const [assessmentData, enrollmentsData, scoresData, filesData, assessmentsData] = await Promise.all([
      ...
      if (assessmentsData.length > 0) {
        const allScoresPromises = assessmentsData.map((a: Assessment) =>
    ...
    return (
      <div className="flex h-full flex-col gap-4">
        ...
        <div className="flex flex-1 gap-4">
          <Skeleton className="w-[45%] rounded-xl" />
