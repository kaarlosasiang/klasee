...
/**
 * Uploads a raw document (PDF, DOCX, etc.) to Cloudinary using the
 * `raw/upload` endpoint and returns the secure URL.
 */
export async function uploadDocumentToCloudinary(file: File): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(`Cloudinary document upload failed: ${res.statusText}`)
  }
